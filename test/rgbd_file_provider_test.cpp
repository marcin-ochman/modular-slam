...
SCENARIO("Data provider is RgbdFileProvider")
{
    GIVEN("RgbdFileProvider instance with specified paths to rgb and depth data after fetch")
    {
        ...
        // RgbdFileProvider fileProvider{{rgbDirPath, depthDirPath}, cameraParams};
        ...
        //             static_cast<std::size_t>(refMatRgb.size().height * refMatRgb.size().width *
        //             refMatRgb.channels()));
        ...
        //             static_cast<std::size_t>(refMatDepth.size().height * refMatDepth.size().width));
        //     REQUIRE(
        //         std::equal(nextRgbd->depth.data.begin(), nextRgbd->depth.data.end(),
